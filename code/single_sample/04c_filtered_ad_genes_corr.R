...
input_dir <- here("corr_outputs",
                   sample_ids[3])
res_list <- readRDS( paste0(input_dir, "/non_zero_PAbeta.RDS"))
...
## loop for removing zeros from gene as well as pAbeta
...
pdf(file = paste0(output_dir, "/", "filtered_ad_pearson_PAbeta.pdf"))
for( i in seq_len(nrow(non_zero_pabeta_df))){
    gene_name = non_zero_pabeta_df$gene_id[i]
    ...
    p <- ggplot(data=as.data.frame(res_list[[gene_name]]), aes(x=x, y= y)) +
        ...
        ggtitle(paste0('Absolute Pearson Correlation = ', as.character(non_zero_pabeta_df$abs_corr[i])))
...
output_dir <- here("plots","02_sample_subset", sample_ids[3])
pdf(file = paste0(output_dir, "/", "filtered_ad_spearman_PAbeta.pdf"))
for( i in seq_len(nrow(non_zero_pabeta_df))){
    gene_name = non_zero_pabeta_df$gene_id[i]
    ...
    p <- ggplot(data=as.data.frame(res_list[[gene_name]]), aes(x=x, y= y)) +
        ...
        stat_density(aes(x=x, y=((..scaled..))), position="identity", geom="line") +
        xlab(gene_name) + ylab('Percentage Abeta') +
        ggtitle(paste0('Absolute Spearman Correlation = ', as.character(non_zero_pabeta_df$abs_corr[i])))
