# library(sgejobs)
# sgejobs::job_single('non_zero_pTau',
#                     create_shell = TRUE,
#                     create_logdir = TRUE, task_num = 10,
#                     queue = "bluejay",
#                     cores = 5L,
#                     command = "Rscript non_zero_pTau.R",
#                     memory = "20G")
...
names(PpTau) <- rownames(colData(spe_sub))
...
output_dir <- here("corr_outputs",
                   sample_ids[s])
saveRDS(res_list, paste0(output_dir, "/non_zero_PpTau.RDS"))
